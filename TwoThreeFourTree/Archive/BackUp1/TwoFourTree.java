...
public class TwoFourTree {
    ...
    public boolean addValue(int value) {
    	...
    	while(tranversingNode!=null) {
    		 ...
    		 */
    		tranversingNode = getNextNode(tranversingNode, value);
    	}
    	return false;
    }
    ...
    public boolean hasValueRecursive(TwoFourTreeItem node, int value) {
    	// End Cases
    	if(node==null)return false;
    	if(doesCurrentNodeHasValue(node, value)) return true;
    	
    	return hasValueRecursive(getNextNode(node, value), value);
    }
    
    /**
     * Checks whether the current node has the targeted value
     * @param node - The node that is being checked
     * @param value - The value that is checking
     * @return true if the node has value, otherwise false
     */
    public boolean doesCurrentNodeHasValue(TwoFourTreeItem node, int value) {
    	if(node==null)return false;
     ...
     */
    public TwoFourTreeItem getNextNode(TwoFourTreeItem node, int value) {
    	if(node.isTwoNode()) {
			if(value<node.value1) return node.leftChild;// go to left node
			else if(value>node.value1) return node.rightChild;// go to right node
		}else if(node.isThreeNode()) {
			if(value<node.value1) return node.leftChild;// go to left node
			else if(value>node.value1 && value<node.value2) return node.centerChild;// go to center node
			else if(value>node.value2) return node.rightChild;// go to right node
		}else if(node.isFourNode()) {
			// Four Node
			if(value<node.value1) return node.leftChild;// go to left node
			else if(value>node.value1 && value<node.value2) return node.centerLeftChild;// go to center left node
			else if(value>node.value2 && value<node.value3) return node.centerRightChild;// go to center right node
			else if(value>node.value3) return node.rightChild;// go to right node
		}
    ...
    public boolean deleteValue(int value) {
    	...
    	while(deletionNode!=null) {
    		...
    		if(doesCurrentNodeHasValue(deletionNode,value)) {
    			...
    			TwoFourTreeItem nextNode= getNextNode(deletionNode, value);
